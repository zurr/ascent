...
void ArathiBasin::HandleBanner(Player *p_caster,GameObject *go,uint32 spellid)
{
	...
	if(bannerslot > -1 && bannerslot < 5)
	{
		...
		sEventMgr.AddEvent(this, &ArathiBasin::EventResourcesCapture, p_caster->GetGUID(), (uint32)bannerslot, EVENT_BATTLEGROUND_STABLE_RCONTEST + bannerslot, 60000, 1);
	}
}
...
void ArathiBasin::EventResourcesCapture(uint64 srcguid,uint32 bannerslot)
{   
	Player * src = m_MapMgr->GetPlayer(srcguid);
	if(!src) return;
