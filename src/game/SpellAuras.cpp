...
void Aura::SpellAuraModPossess(bool apply)
{
	...
	if(m_spellProto->Id == 24937)
	{
		...
		{
			// spawn rc
			target = m_target->GetMapMgr()->CreateCreature();
			...
			target->SetUInt32Value(UNIT_FIELD_BYTES_0,  ((0x02 << 8) | (0x02 << 16)));
			...
			//target->AddToWorld();
			target->PushToWorld(caster->GetMapMgr());
		} 
		else 
		{
			target = caster->GetMapMgr()->GetCreature(caster->GetUInt32Value(UNIT_FIELD_CHARM));
			if(!target)
				return;
		}		
	}
	...
	else 
	{
		...
		static_cast<Player*>(caster)->m_CurrentCharm = NULL;
