...
{
	// this is not yet perfect, its possible to crash the client if the user is cheating. Anyways who cares if his client crashes
	if (StatusID == 0)
	{
...
void BattlegroundMgr::CreateWSGBattleground()
{
	...
	LevMin = 1;
	LevMax = sWorld.Expansion1LevelCap;
...
#define RESURRECT_SPELL 21074   // Spirit Healer Res
	...
	{
		Player *SelectedPlayer = m_MapMgr->GetPlayer(itr->second);
		if(SelectedPlayer)
		{
			if(SelectedPlayer->isDead())
			{
				Unit *sh = m_MapMgr->GetCreature(itr->first);
				if(sh)
				{
...
void Battleground::EventRemove()
{
	...
	switch(m_MapId)
	{
		case 489:
		{
			...
			(uint32)489, EVENT_BATTLEGROUND_REMOVE, 10000, 1);
		}break;
		case 529:
		{
			...
			sEventMgr.AddEvent(BattlegroundMgr::getSingletonPtr(), &BattlegroundMgr::EventCreateBattleground,
			(uint32)529, EVENT_BATTLEGROUND_REMOVE, 10000, 1);
		}break;
		case 30:
		{
			((AlteracValley*)this)->Remove();
			sEventMgr.AddEvent(BattlegroundMgr::getSingletonPtr(), &BattlegroundMgr::EventCreateBattleground,
			(uint32)30, EVENT_BATTLEGROUND_REMOVE, 10000, 1);
		}break;
		case 562:
		{
			((Arena*)this)->Remove();
			sEventMgr.AddEvent(BattlegroundMgr::getSingletonPtr(), &BattlegroundMgr::EventCreateBattleground,
			(uint32)562, EVENT_BATTLEGROUND_REMOVE, 10000, 1);
		}break;   
	}   
	
	RemoveAllPlayers(true,true);	
	//sEventMgr.AddEvent(World::getSingletonPtr(), &World::EventDeleteBattleground, this, EVENT_BATTLEGROUND_REMOVE, 1, 1);

	// Kill battleground thread. This will cause it to delete.
...
void Battleground::End()
{
	...
	sEventMgr.AddEvent(this, &Battleground::EventRemove, EVENT_BATTLEGROUND_REMOVE, 20000,1); // remove after 2mins
...
bool Battleground::GetRepopCoordinates(Player * plr, LocationVector & vec)
{
	return false;
}
