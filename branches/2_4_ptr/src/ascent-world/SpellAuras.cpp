...
void Aura::SpellAuraModAttackSpeed(bool apply)
{
	...
	else
	{
		if(apply)
		{
			...
			mod->fixed_amount[1] = m_target->GetModPUInt32Value(UNIT_FIELD_BASEATTACKTIME_2,mod->m_amount);
			...
			m_target->ModUInt32Value(UNIT_FIELD_BASEATTACKTIME_2, -mod->fixed_amount[1]);
...
void Aura::SpellAuraModHaste( bool apply )
{
	...
	else
	{
		if( apply )
		{
			...
			if( (int32)m_target->GetUInt32Value ( UNIT_FIELD_BASEATTACKTIME_2 ) <= mod->fixed_amount[1] )
				mod->fixed_amount[1] = m_target->GetUInt32Value ( UNIT_FIELD_BASEATTACKTIME_2 );//watch it, a negative timer might be bad ;)
