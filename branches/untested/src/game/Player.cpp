...
{   
	...
	int32 bonus ;
	int32 stat_bonus;
	stat_bonus = GetUInt32Value( UNIT_FIELD_POSSTAT2 ) - GetUInt32Value( UNIT_FIELD_NEGSTAT2 );
	if ( stat_bonus < 0 )
		stat_bonus = 0; //avoid of having negative health
	bonus = stat_bonus * 10 + m_healthfromspell + m_healthfromitems;
		...
		case TYPEID_PLAYER:
			{
				...
				if(pObj->IsStealth()) // Stealth Detection (  I Hate Rogues :P  )
				{
					...
					if(isInFront(pObj)) // stealthed player is in front of us
					{
						...
						if(getLevel() < 60)
							detectRange = 5.0f + getLevel() + 0.2f * (float)(getLevel() + GetStealthDetectBonus() - pObj->GetStealthLevel());
						else
...
void Player::EndDuel(uint8 WinCondition)
{
	...
	//get Arbiter
	GameObject* arbiter = m_mapMgr ? GetMapMgr()->GetGameObject( GetUInt32Value( PLAYER_DUEL_ARBITER ) ) : NULL;
